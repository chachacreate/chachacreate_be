...
public interface ClassInfoRepository extends JpaRepository<ClassInfoEntity, Long> {
	...
	/**
	 * 날짜 기준 예약 가능 클래스 조회 - 선택한 날짜의 클래스 - 여석 = participant - 예약 수 - startDate +
	 * startTime 오름차순 정렬
	 */
	@Query("""
			SELECT new com.create.chacha.domains.shared.classes.vo.ClassCardVO(
			...
			""")
	List<ClassCardVO> findAvailableClassesByDate(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
